...
def searchAjax(request):
    ...
    if nombres != '':
        datas = datas.filter(nombres=nombres)
    if escolaridad != '0':
    ...
    if empresa != '':
        datas = datas.filter(empresa=empresa)
    if no_emp != '':
        datas = datas.filter(no_emp=no_emp)
    if apellido_p != '':
        datas = datas.filter(apellido_p=apellido_p)
    if genero != '0':
